...
const DEFAULTS = {
  ...
  maxRepairRounds: 2,
  maxPolishRounds: 1,
  model: process.env.FRONTEND_AUTOPILOT_MODEL || 'gpt-4o-mini',
  baseUrl: process.env.FRONTEND_AUTOPILOT_BASE_URL || 'https://api.openai.com/v1',
  userAgent: process.env.FRONTEND_AUTOPILOT_USER_AGENT || '',
  temperature: 0.2,
  ...
  maxNetworkRetries: Number(process.env.FRONTEND_AUTOPILOT_MAX_RETRIES) || 6,
  requestTimeoutMs: Number(process.env.FRONTEND_AUTOPILOT_REQUEST_TIMEOUT_MS) || 120_000,
  streamRequestTimeoutMs: Number(process.env.FRONTEND_AUTOPILOT_STREAM_TIMEOUT_MS) || 600_000,
  visualThreshold: Number(process.env.FRONTEND_AUTOPILOT_VISUAL_THRESHOLD) || 0.62,
};
...
export async function loadConfig(targetDir, overrides = {}) {
  ...
  const apiKey = overrides.apiKey || process.env.FRONTEND_AUTOPILOT_API_KEY;
  if (!apiKey) {
    throw new Error('FRONTEND_AUTOPILOT_API_KEY is not set (see .env.example)');
  }
