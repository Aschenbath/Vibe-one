...
1. **Capture ONE successful real run + its screenshots.** This is the only thing between the repo and the FRAMEWORK interview bar. The pipeline is proven offline (e2e) and the planner + builder + repair loop all work against the real model (opus-4-8). The blocker has been purely external: the `api.123nhh.com` gateway was unstable for hours on 2026-07-10 — small requests returned 200, but the long build-response call kept hitting multi-minute windows of `fetch failed` + 429 that exhausted even 6 retries. Runs 3-6 all died at the build model call for this reason, not a code fault. **Resume checklist:**
   - Probe first: `curl -m 15 -H "Authorization: Bearer $VIBE_ONE_API_KEY" https://api.123nhh.com/v1/chat/completions -H "content-type: application/json" -d '{"model":"claude-opus-4-8","messages":[{"role":"user","content":"hi"}]}'` a few times. If it's consistently 200 in <5s, proceed. If not, get a fresh non-shared key from Gilbert or wait.
   - `node src/cli/index.js plan examples/expense-mobile` (one cheap call) to confirm the gateway is healthy for real work.
   - `npm run demo:expense`. It should reach `status: success` (build -> preview -> screenshots -> scenarios -> review).
   - Copy `runs/<id>/screenshots/*.png` into `docs/screenshots/`, embed them in README (a "Generated app" section), and commit. That clears "screenshots in repo" + one successful demo.
   - Then repeat for `examples/notes-mobile` (second demo) and keep one failed-then-repaired run's `DELIVERY_REPORT.md` for the interview narrative.
...
3. **Fixer prompt is untested against a PASSING repair.** The fixer now sees the source (`gatherSource`) and uses the delimiter protocol, but a real failed-then-repaired-to-green cycle hasn't been captured (gateway blocked it). Watch the first real repair: if it drops the fixed scaffold or returns partial files, add a guard that re-writes the scaffold before each verify pass (currently only written once at build).
