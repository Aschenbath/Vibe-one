# SignalDesk：AI 生成之后，我让程序自己验收

SignalDesk 是一个客服质检后台，也是 Vibe-one 目前最完整的一次真实生成案例。

我没有先手写页面再让工具“演示生成”。输入只有一份产品需求：做一个包含运营总览、风险会话队列和质检详情的前端，让主管能找到高风险会话、查看证据、分配负责人并完成复核。
...
这次过程最能说明 Vibe-one 的价值：**生成并不是结束，发现问题、控制修改范围、证明没有改坏，才是交付。**
